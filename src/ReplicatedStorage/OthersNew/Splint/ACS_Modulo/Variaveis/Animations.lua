...
local event = game.ReplicatedStorage.ACS_Engine.Eventos.MedSys.Splint
local value = 1
local module = {
...
--//Client Animations
	...
	ChamberAnim = function(char, speed, objs)
		...
		ts(objs[3],TweenInfo.new(0.5),{C1 = CFrame.new(0.15,0.1,-1.2) * CFrame.Angles(math.rad(-120),math.rad(15),math.rad(15))}):Play()
			...
			ts(objs[2],TweenInfo.new(0.6),{C1 = require(script.Parent.Settings).LeftPos}):Play()
			ts(objs[3],TweenInfo.new(0.6),{C1 = require(script.Parent.Settings).LeftPos}):Play()
			event:FireServer()
			value = value - 1
			wait(4.5)
		elseif value <= 0 then
