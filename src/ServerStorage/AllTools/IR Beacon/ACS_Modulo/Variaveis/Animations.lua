...
local event = game.ReplicatedStorage.DropEvent
local value = 1
local module = {
...
--//Client Animations
	...
	ChamberAnim = function(char, speed, objs)
		if value > 0 then
			...
			ts(objs[2],TweenInfo.new(0.35),{C1 = CFrame.new(-0.875, 0, -1.15) * CFrame.Angles(math.rad(-95), math.rad(-2), math.rad(7.5))}):Play()
			
		wait(.25)
		
			script.Parent.Parent.Parent.Parent:FindFirstChild('S'..objs[5].Name).Grip:WaitForChild("Switch"):Play()
			ts(objs[2],TweenInfo.new(0.3),{C1 = CFrame.new(-0.875, 0.5, -1.35) * CFrame.Angles(math.rad(-100), math.rad(-2), math.rad(7.5))}):Play()
			wait(0.35)
			local MagC2 = objs[5]:WaitForChild("beacon"):clone()
			objs[5].beacon.Transparency = 1
			...
			MagCW2.Part0 = MagC2
			MagCW2.Part1 = objs[3].Parent.Parent:WaitForChild("Right Arm")
			MagCW2.Parent = MagC2
			MagCW2.C1 = MagC2.CFrame:toObjectSpace(objs[5].beacon.CFrame)
			--ts(MagCW,TweenInfo.new(0),{C0 = CFrame.new(0.21, -0.4, -0.75) * CFrame.Angles(math.rad(90), math.rad(0), math.rad(180))}):Play()
			
			script.Parent.Parent.Parent.Parent:FindFirstChild('S'..objs[5].Name).Grip:WaitForChild("Place"):Play()
			...
			event:FireServer(MagC2.Position)
			value = value - 1
			wait(.15)
			if value <= 0 then
