...
--//Client Animations
	...
	ChamberAnim = function(char, speed, objs)
		...
		wait(.5)
		ts(objs[3],TweenInfo.new(0.15),{C1 = require(script.Parent.Settings).LeftPos}):Play()
		ts(objs[2],TweenInfo.new(0.3),{C1 = CFrame.new(-0.1, -1.75, -.75)*CFrame.Angles(math.rad(-180), 0, 0)*CFrame.Angles(0, 0, math.rad(-30))}):Play()
		wait(0.5)
		script.Parent.Parent.Parent.Parent:FindFirstChild('S'..objs[5].Name).Grip:WaitForChild("Drink"):Play()
		if player.ThirstVal then
			local value = player.ThirstVal.Value
			if player.ThirstVal.Value + 50 >= 100 then
				player.ThirstVal.Value = 100
				print(value)
					print("set to 100")
					
			else
				player.ThirstVal.Value = player.ThirstVal.Value + 50
				print(player.ThirstVal.Value)
				print("added")
				end
				end
