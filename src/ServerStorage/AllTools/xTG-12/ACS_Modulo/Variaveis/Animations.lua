...
local event = game.ReplicatedStorage.ACS_Engine.Eventos.MedSys.Antidote
local value = 1
local module = {
...
--//Client Animations
	...
	ChamberAnim = function(char, speed, objs)
		if value > 0 then
			...
			wait(.35)
			local MagC = objs[5]:WaitForChild("Cap"):clone()
			objs[5].Cap.Transparency = 1
			...
			MagCW.Parent = MagC
			MagCW.C1 = MagC.CFrame:toObjectSpace(objs[5].Cap.CFrame)
		...
		script.Parent.Parent.Parent.Parent:FindFirstChild('S'..objs[5].Name).Grip:WaitForChild("Open"):Play()
			...
			game:GetService('Debris'):AddItem(MagC,10)
			
		wait(.25)
		
			script.Parent.Parent.Parent.Parent:FindFirstChild('S'..objs[5].Name).Grip:WaitForChild("Use"):Play()
			ts(objs[2],TweenInfo.new(0.3),{C1 = CFrame.new(-.45, -0.42, -1) * CFrame.Angles(math.rad(-90), math.rad(0), math.rad(-3))}):Play()
			ts(objs[3],TweenInfo.new(0.45),{C1 = CFrame.new(0.45,0.05,-1.25) * CFrame.Angles(math.rad(-100),math.rad(80),math.rad(7.5))}):Play()
			event:FireServer(script.Parent.Parent.Parent.Name)
			value = value - 1
			wait(0.6)
			ts(objs[3],TweenInfo.new(0.25),{C1 = CFrame.new(0.15,-0.1,-1.2) * CFrame.Angles(math.rad(-120),math.rad(15),math.rad(15))}):Play()
			
			ts(objs[2],TweenInfo.new(0.175),{C1 = CFrame.new(-0.875, 0.5, -1.15) * CFrame.Angles(math.rad(-95), math.rad(-2), math.rad(7.5))}):Play()
			ts(objs[3],TweenInfo.new(0.175),{C1 = CFrame.new(-0.5,0.475,-1.6) * CFrame.Angles(math.rad(-100),math.rad(0),math.rad(7.5))}):Play()
			wait(.175)
			local MagC2 = objs[5]:WaitForChild("Body"):clone()
			objs[5].Body.Transparency = 1
			MagC2.Parent = workspace.CurrentCamera
			MagC2.Name = "MagC"
			MagC2.Transparency = 0			

			local MagCW2 = Instance.new("Motor6D")
			MagCW2.Part0 = MagC2
			MagCW2.Part1 = objs[3].Parent.Parent:WaitForChild("Left Arm")
			MagCW2.Parent = MagC2
			MagCW2.C1 = MagC2.CFrame:toObjectSpace(objs[5].Body.CFrame)
			--ts(MagCW,TweenInfo.new(0),{C0 = CFrame.new(0.21, -0.4, -0.75) * CFrame.Angles(math.rad(90), math.rad(0), math.rad(180))}):Play()
			
			wait(.1)
			script.Parent.Parent.Parent.Parent:FindFirstChild('S'..objs[5].Name).Grip:WaitForChild("Throw"):Play()
			ts(objs[3],TweenInfo.new(0.2),{C1 = CFrame.new(0.15,0.1,-1.2) * CFrame.Angles(math.rad(-120),math.rad(15),math.rad(15))}):Play()
			MagC2:ClearAllChildren()
			MagC2.Parent = workspace
			MagC2.Anchored = false
			MagC2.CanCollide = true
			MagC2.RotVelocity = Vector3.new(0,25,0)
			MagC2.Velocity = (MagC2.CFrame.LookVector * 15) + (MagC2.CFrame.RightVector * 5) + Vector3.new(0,25,0)
			game:GetService('Debris'):AddItem(MagC,10)
