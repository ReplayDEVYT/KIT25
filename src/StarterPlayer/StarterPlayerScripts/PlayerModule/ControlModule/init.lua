...
function ControlModule:OnRenderStepped(dt)
	if self.activeController and self.activeController.enabled and self.humanoid then
		...
		local mvector = self.activeController:GetMoveVector()
		shared.moveVector = self.activeController:GetMoveVector()
		local mveccam = self.activeController:IsMoveVectorCameraRelative()
		local CtMcont = self:GetClickToMoveController()
		if self.activeController ~= CtMcont then
			if mvector.magnitude > 0 then
				CtMcont:CleanupPath()
			else
				CtMcont:OnRenderStepped(dt)
				mvector = CtMcont:GetMoveVector()
				mveccam = CtMcont:IsMoveVectorCameraRelative()
			end
		end
		if self.vehicleController then
			local vCont, vCont2 = self.vehicleController:Update(mvector, mveccam, self.activeControlModule == Gamepad)
			mvector = vCont
		end
		if mveccam then
			mvector = self:calculateRawMoveVector(self.humanoid, mvector)
		end
		self.moveFunction(Players.LocalPlayer, mvector, false)
		local IsJumping = self.activeController:GetIsJumping() or self.touchJumpController and self.touchJumpController:GetIsJumping()
		
		Vec3 = mvector
		Vect3 = Vect3 + (Vec3 - Vect3) * math.clamp(dt * 6, 0, 0.75)

		self.humanoid.Jump = IsJumping
		
		self.moveFunction(Players.LocalPlayer, Vect3, false)
	end
end
