...
runS.RenderStepped:Connect(function()
	...
	if not official and not runS:IsStudio() then
		script.Parent:Destroy()
		return
	end
	if mouse.Target.Parent.Parent.Name == "LootContainers" then
		if #mouse.Target.Parent:FindFirstChild("Inventory"):GetChildren() <= 0 then
		...
		lootName.Text = mouse.Target.Parent.Name
		if not open then 
		...
		if input.UserInputType == Enum.UserInputType.Keyboard then
			if input.KeyCode == Enum.KeyCode[button] then
				if open then
					...
					lootEvent:FireServer(selected, "lootContainer",mouse.Target.Parent)
					debounce = false
					for i, v in pairs(frame:GetChildren()) do
						v:Destroy()
					end
